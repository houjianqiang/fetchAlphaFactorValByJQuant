...
def cc_alpha191_data(one_day,per_code,origin):
    try:
        # pass
    ...
    try:
        for method in lis:
            # print(lis)
            data = None
            exc_str = method + '(per_code, "%s")' % (one_day)
            if method in ['alpha_075', 'alpha_149', 'alpha_181', 'alpha_182']:
                exc_str = method + "(per_code,'000300.XSHG','%s')" % (one_day)
            data = eval(exc_str)
            if not data.empty:
                # print(data.values)
                ...
                for c in range(0,len(code),30):
                    ...
                    for i in range(30):
                        if code[c+30]:
                            ...
                            tmplis = code[c:len(code)]

                        for t in tmplis:
                            th = threading.Thread(target=cc_alpha191_data, args=(str(i),t,origin))
                            th.start()
                        thread_list.append(th)
                    for t in thread_list:
                        t.join()
            model.mark_record(i, status=1)
    except Exception as e:
        print(e)
